% Codes of algorithms tested: inverse update with I as weighted matrix

%% Function: main function inverseUpdate

function [x1,out] = nmiu1(myFx, x0, gradToler, maxIter, colSize, Q)
% Input
...
taulst = 0;
    ...
    % loop for optimization algorithm
    while true
      ...
      if (k <= (colSize/2)) 
          ...
          u_tau = s0-tau*y0;
          bdd = const_yts-tau*const_yty;
          vec_Uty = U'*y0;
          % sub-pquestion: quadratic minimization spherical constraint
          A = U'*U-(1/const_yty)*(vec_Uty*vec_Uty'); % disp(det(A)),
          beta = -U'*u_tau; 
          vstar = MoreSorensen(A,beta,sqrt(bdd));  % trust region algorithm to find v* 
          vec_uvs = U*vstar;
          % update low-rank matrix U
          U = U + ((y0'*vec_uvs)/(bdd*const_yty)*y0+(1/bdd)*...
              (u_tau-vec_uvs))*vstar' - 1/const_yty*y0*(vec_Uty'); 

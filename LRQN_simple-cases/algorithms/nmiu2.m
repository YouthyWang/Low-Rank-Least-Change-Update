% Codes of algorithms tested: inverse update 2

%% Function: main function inverseUpdate

function [x1,out] = nmiu2(myFx, x0, gradToler, maxIter, colSize, Q)
% Input
...
taulst = 1;
    ...
    while true
      ...
      if (k <= (colSize/2)) 
          ...
          u_tau = s0-tau*y0;
          bdd = sqrt(const_yts-tau*const_yty);
          vec_Uy = U'*y0;
          % trust region algorithm to find v*
          A = U'*U*(taulst*Im+U'*U)^(-1)-1/(taulst*(const_yty)+vec_Uy'*vec_Uy)...
              *(vec_Uy)*(vec_Uy');
          beta = -(taulst*Im+U'*U)^(-1)*(U'*u_tau);
          vstar = MoreSorensen(A,beta,bdd); 
          % prepare the fixed coefficients and vectors
          const_ymy = tau*const_yty+vec_Uy'*vec_Uy;
          const_uy  = bdd^2;
          vec_my = tau*y0+U*vec_Uy;
          vec_Uv = U*vstar;
          % update low rank matrix U
          U = U + ((y0'*vec_Uv)/(const_uy*const_ymy)*vec_my+(1/const_uy)*...
              (u_tau-vec_Uv))*(vstar') - 1/const_ymy*vec_my*(vec_Uy');
